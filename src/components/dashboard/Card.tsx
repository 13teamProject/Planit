...
function Card({ card, columnId, onDragStart, onClick, isDragging }: CardProps) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      role="presentation"
      className={`min-h-100 cursor-grab rounded-8 border bg-white p-10 transition-all duration-200 dark:bg-gray-700 ${
        isDragging
    ...
    >
      ...
      <div className="flex flex-grow flex-col justify-between">
        <h1 className="overflow-hidden text-ellipsis whitespace-nowrap py-4 dark:text-white sm:max-w-150 sm:text-14 md:max-w-500 md:text-16 lg:max-w-300">
          {card.title}
        </h1>
            ...
            <div className="flex items-baseline gap-4">
              {card.dueDate && (
                <>
                  ...
                  <p className="flex-grow text-gray-300 dark:text-white sm:text-10 md:text-12">
